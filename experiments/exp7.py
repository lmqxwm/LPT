...
if __name__ == '__main__':
    ...
    with pool:
        for t in range(len(thes)):
            ...
            result = pool.map(partial(simufunc.experiment5, Mt=mt, N=100, M=m, theta=thes[t]), 
                [i for i in range(100)])
            results[0, t] = np.mean([r[0] for r in result])
            results[1, t] = np.mean([r[1] for r in result])
            pd.DataFrame(results, columns=thes, index=["Cor_kernel", "Linear_reg"]).to_csv("result7.csv")